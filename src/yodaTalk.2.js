...
$(window.document).ready(function () {
    ...
    HtmlStack.prototype.push = function (element, force) {
        ...
        // allow to add text and result if needed (ex. initial value)
        if (this.stack.length <= 1 || force) {
            ...
            //console.log("...Initial",text, result); 
            current.text = text;
    ...
    HtmlStack.prototype.getCurrentElement = function () {
        console.log("...index [result]", this.getCurrentIndex());
    ...
    HtmlStack.prototype.slowTextInput = function (element, text) {
        ...
        var textElement = $(element).children(".conversion_div")[0];
        
        
        var promise = deferred.promise.when(
            teletype(textElement, text, 20, true, function (i, l) {
    ...
    // obtain the data: text, language, etc
    
    var htmlElement = (function (element) {
        return {
            element: element,
            progress: $(element).children(".yoda"),
            language:  $(element).children(".selectoid").children(".selectoid_select").find(":selected").val(),
    ...
    function initiateTranslationSequence () {
        ...
        var translationStack = new HtmlStack(".conversion");
        ...
        for (var i = length; i > 1; i --) {
            nextTick = nextTick.then(processItem, undefined, function () { console.log("some progress"); } );
        }
